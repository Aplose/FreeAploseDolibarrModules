...
/**
 * \file    dolicarbon/admin/setup.php
 * \ingroup dolicarbon
 * \brief   DoliCarbon setup page.
 */
...
require_once '../lib/dolicarbon.lib.php';
...
// Translations
$langs->loadLangs(array("admin", "dolicarbon@dolicarbon"));

// Initialize a technical object to manage hooks of page. Note that conf->hooks_modules contains an array of hook context
/** @var HookManager $hookmanager */
$hookmanager->initHooks(array('dolicarbonsetup', 'globalsetup'));
...
// DoliCarbon parameters
$formSetup->newItem('DOLICARBON_TRIGGER_NOTIFY')->setAsYesNo();
$formSetup->newItem('DOLICARBON_AUTO_IMPORT_SUPPLIER_INVOICE')->setAsYesNo();
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/dolicarbon/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
	...
	if ($className !== '') {
		...
		if ($module->write_file($tmpobject, $langs) > 0) {
			header("Location: ".DOL_URL_ROOT."/document.php?modulepart=dolicarbon-".strtolower($tmpobjectkey)."&file=SPECIMEN.pdf");
			return;
		} else {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'DOLICARBON_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		if (!empty($tmpobjectkey)) {
			$constforval = 'DOLICARBON_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
$help_url = '';
$title = $langs->trans("DoliCarbonSetup");

llxHeader('', $langs->trans($title), $help_url, '', 0, 0, '', '', '', 'mod-dolicarbon page-admin');
...
$head = dolicarbonAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', $langs->trans($title), -1, "dolicarbon@dolicarbon");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("DoliCarbonSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'DOLICARBON_'.strtoupper($myTmpObjectKey).'_ADDON';
							...
							if (preg_match('/\.modules\.php$/i', $file) && preg_match('/^(pdf_|doc_)/', $file)) {
								if (file_exists($dir.'/'.$file)) {
									...
									if ($modulequalified) {
										...
										// Default
										print '<td class="center">';
										$constforvar = 'DOLICARBON_'.strtoupper($myTmpObjectKey).'_ADDON_PDF';
