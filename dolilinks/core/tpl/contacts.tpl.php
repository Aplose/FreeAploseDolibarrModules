...
// Nom de la méthode de selection de contact en fonction de la version de Dolibarr
$selectContactFunctionName = DOL_VERSION < 21 ? "selectcontacts" : "select_contact";
...
if ($permission) {
	...
	?>
		...
		<form class="tagtr pair nohover" action="<?php echo $_SERVER["PHP_SELF"] . '?id=' . $object->id; ?>" method="POST">
			...
			<div class="nowrap tagtd"><?php echo img_object('', 'contact') . ' ' . $langs->trans("ThirdPartyContact"); ?></div>
			<div class="tagtd nowrap ">
				<?php
			...
			<div class="tagtd  minwidth500imp">
				<?php
				...
				print $form->$selectContactFunctionName(($selectedCompany > 0 ? $selectedCompany : -1), '', 'contactid', 3, '', '', 1, 'minwidth100imp widthcentpercentminusxx maxwidth400');
		...
		<form
			...
			<div class="tagtd noborderbottom">
				...
				print " ".$langs->trans("ChildThirpartyContact");
				?>
			</div>
			...
			<div class="tagtd nowrap noborderbottom">
				...
				$childs = $societeLink->getChilds($object->thirdparty->id);
				$childsNamesByIds = [];
				foreach ($childs as $child) {
					$childsNamesByIds[$child->id] = $child->name;
				}
				$selectedChildSocieteId = GETPOSTISSET('child-id') ? GETPOSTINT('child-id') : $childs[0]->id;
				// new Form($db);
				print $formcompany->selectArray(
					'child-id',
					$childsNamesByIds,
					$selectedChildSocieteId,
					0,
					0,
					0,
					'style="width:300px"',
					...
					0,
					'',
					'minwidth300px maxwidth400 widthcentpercentminusx'
				);
				?>
				<script>
					document.getElementById('child-id').onchange = function(e) {
						window.location.href = '<?php echo $_SERVER["PHP_SELF"] . '?id=' . $object->id; ?>&child-id=' + this.value;
						console.log('this.value:', this.value['id']);
					};
				</script>
			</div>

			<!-- Colone Utilisateurs|Contacts -->
			<div class="tagtd noborderbottom minwidth500imp">
				<?php
				print img_object('', 'contact', 'class="pictofixedwidth"');
				//print $form->selectcontacts(($selectedCompany > 0 ? $selectedCompany : -1), '', 'contactid', 3, '', '', 1, 'minwidth100imp widthcentpercentminusxx maxwidth400');
				print $form->$selectContactFunctionName(
					$selectedChildSocieteId > 0 ? $selectedChildSocieteId : -1,
					'',
					'contactid',
					0,
					'',
					'',
					1,
					'minwidth100imp maxwidth400 widthcentpercentminusx'
				);
				$nbofcontacts = $form->num;
				$newcardbutton = '';
				if (!empty($object->socid) && $object->socid > 1 && $user->hasRight('societe', 'creer')) {
					$newcardbutton .= '<a href="' . DOL_URL_ROOT . '/contact/card.php?socid=' . $selectedChildSocieteId . '&action=create&backtopage=' . urlencode($_SERVER["PHP_SELF"] . '?id=' . $object->id) . '" title="' . $langs->trans('NewContact') . '"><span class="fa fa-plus-circle valignmiddle paddingleft"></span></a>';
				}
				print $newcardbutton;
				?>
			...
			<!-- Colone Role de contact -->
			<div class="tagtd noborderbottom">
				<?php
				$formcompany->selectTypeContact(
					$tmpobject,
					2,
					'typecontact',
					'external',
					'position',
					0,
					'minwidth150imp widthcentpercentminusx maxwidth200'
				);
...
foreach (array('internal', 'external') as $source) {
	...
	foreach ($contactlist as $contact) {
		...
		if ($contact['source'] == 'internal') {
			...
			if (in_array($contact['socid'], array_map(fn($item) => $item->id, $childs))) {
				$entry->nature = 'childThirdparty';
				$entry->nature_html = $langs->trans("ChildThirpartyContact");
			} else {
